...
const schema = z.object({
  ...
  LLM_PROVIDER: z.enum(['hybrid', 'trained', 'cloud', 'gemini', 'ollama', 'lmstudio', 'none']).default('hybrid'),
  OLLAMA_BASE_URL: z.string().url().default('http://localhost:11434/v1'),
  OLLAMA_API_KEY: z.string().default('ollama'),
  // Prefer the 8B local model when it is installed. The provider chain adds
  // OLLAMA_FALLBACK_MODEL as a second local leg so a missing/overloaded 8B
  // model does not take the application offline.
  OLLAMA_MODEL: z.string().default('qwen3:8b'),
  OLLAMA_FALLBACK_MODEL: z.string().default('qwen3:4b'),
  OLLAMA_KEEP_ALIVE: z.string().default('30m'),
  OLLAMA_NUM_CTX: z.coerce.number().int().positive().default(8192),
  OLLAMA_NUM_PREDICT: z.coerce.number().int().positive().default(1600),
  OLLAMA_THINK: z.preprocess(value => value === true || value === 'true', z.boolean()).default(false),
