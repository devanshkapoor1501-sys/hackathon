...
3. The default chain is trained Qwen, NVIDIA, Gemini, then Ollama Qwen3-8B (`OLLAMA_MODEL=qwen3:8b`) followed by Qwen3-4B (`OLLAMA_FALLBACK_MODEL=qwen3:4b`). For the trained primary model, set `TRAINED_MODEL_ENABLED=true`, `TRAINED_MODEL_BASE_URL`, `TRAINED_MODEL=<the exact loaded model id>`, and `TRAINED_MODEL_MANIFEST=<path to deployment-manifest.json>`. The manifest must contain `deploymentGate.passed=true`; otherwise the trained provider is intentionally excluded from the chain.
4. For NVIDIA cloud AI, set `NVIDIA_API_KEY=…`. LM Studio remains supported through explicit `LLM_PROVIDER=lmstudio` mode, but is not part of the default fallback order.
